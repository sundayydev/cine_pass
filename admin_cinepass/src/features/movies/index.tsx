...
import { Plus, Search, Pencil, Trash2, Eye, Film, Calendar, Clock, Filter } from "lucide-react"
  ...
  const filteredMovies =
    statusFilter === "all"
      ...
      : allMovies.filter((movie) => {
          const normalizedStatus = movie.status?.toUpperCase()
          if (statusFilter === "SHOWING") {
            return normalizedStatus === "SHOWING" || normalizedStatus === "NOW_SHOWING"
          }
          ...
          <div className="mt-4 flex items-center gap-6 border-t border-border/50 pt-4">
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-muted-foreground">
                {
                  allMovies.filter(
                    (m) => m.status?.toUpperCase() === "SHOWING" || m.status?.toUpperCase() === "NOW_SHOWING",
                  ).length
                }{" "}
                Đang chiếu
              </span>
            ...
            <div className="flex items-center gap-2 text-sm">
              ...
              <span className="text-muted-foreground">
                {allMovies.filter((m) => m.status?.toUpperCase() === "COMING_SOON").length} Sắp chiếu
              </span>
            </div>
            <div className="flex items-center gap-2 text-sm">
              <div className="h-2 w-2 rounded-full bg-slate-400"></div>
              <span className="text-muted-foreground">
                {allMovies.filter((m) => m.status?.toUpperCase() === "ENDED").length} Đã kết thúc
              </span>
            </div>
          </div>
        </Card>
          ...
          <Table>
            <TableHeader>
              <TableRow className="border-border/50 bg-muted/30 hover:bg-muted/30">
                <TableHead className="hidden w-[100px] font-semibold md:table-cell">Poster</TableHead>
                <TableHead className="min-w-[240px] font-semibold">Thông tin phim</TableHead>
                <TableHead className="hidden font-semibold md:table-cell">Thời lượng</TableHead>
                <TableHead className="hidden font-semibold md:table-cell">Khởi chiếu</TableHead>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="h-40 text-center">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-sm font-medium text-destructive">Lỗi khi tải dữ liệu!</p>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="h-40 text-center">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <p className="text-sm font-medium text-foreground">Không tìm thấy phim nào</p>
                      <p className="text-xs text-muted-foreground">Thử điều chỉnh bộ lọc của bạn</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                movieList.map((movie: Movie) => (
                  <TableRow key={movie.id} className="group border-border/30 transition-colors hover:bg-muted/30">
                    <TableCell className="hidden md:table-cell">
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-1.5">
                        <span className="text-base font-semibold leading-tight text-foreground">{movie.title}</span>
                    ...
                    <TableCell className="hidden md:table-cell">
                      <div className="flex items-center gap-2 text-sm text-foreground">
                        ...
                        <span className="font-medium">{movie.durationMinutes}</span>
                    ...
                    <TableCell className="hidden md:table-cell">
                      <div className="flex items-center gap-2 text-sm">
                        ...
                        <span className="font-medium text-foreground">{formatDate(movie.releaseDate || "")}</span>
                      </div>
                    </TableCell>
                    <TableCell>{renderStatusBadge(movie.status as string)}</TableCell>
                    <TableCell>
          ...
          {!isLoading && !isError && totalItems > 0 && (
            <div className="flex flex-col gap-4 border-t border-border/50 bg-muted/20 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="text-sm text-muted-foreground">
                Hiển thị <span className="font-semibold text-foreground">{startIndex + 1}</span> -{" "}
                <span className="font-semibold text-foreground">{Math.min(endIndex, totalItems)}</span> trong tổng số{" "}
                <span className="font-semibold text-foreground">{totalItems}</span> phim
              </div>
